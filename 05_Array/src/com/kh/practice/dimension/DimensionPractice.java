...
public class DimensionPractice {
	...
	public void practice4() {
		...
		for (int i = 0; i < rawLength - 1; i++) {
			for (int j = 0; j < lineLength - 1; j++) {
				...
				// 행들의 합
				arr[i][lineLength - 1] += arr[i][j];
				// 열들의 합
				arr[rawLength - 1][j] += arr[i][j];

				// 가로세로합
				arr[rawLength - 1][lineLength - 1] += arr[i][j] * 2;
			}
		}
	...
	public void practice5() {
		...
		while (true) {
			...
			if ((0 < raw && raw < 11) && (0 < line && line < 11)) {
				break;
			} else {
				System.out.println("반드시 1~10사이의 정수를 입력해야 합니다.");
			}
		}
		char[][] cArr = new char[raw][line];
		for (int i = 0; i < raw; i++) {
			...
			for (int i = 0; i < classPartition1.length; i++) {
				for (int j = 0; j < classPartition1[i].length; j++) {
					if (classPartition1[i][j].equals(search)) {
						System.out.printf("검색하신 %s 학생은 1분단 %c 번째 줄 %s에 있습니다.", search, line[i], raw[j]);
						break Loop1;
					}
				}
			}
			for (int i = 0; i < classPartition2.length; i++) {
				for (int j = 0; j < classPartition2[i].length; j++) {
					if (classPartition2[i][j].equals(search)) {
						System.out.printf("검색하신 %s 학생은 2분단 %c 번째 줄 %s에 있습니다.", search, line[i], raw[j]);
						break Loop1;
					}
				}
			}
